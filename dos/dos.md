...
**错误代码判断格式：`if [not] errorlevel 错误代码 要执行的命令`**
...
| 常用 | 命令                                             | 解释                             |
| ---- | ------------------------------------------------ | -------------------------------- |
| ■■   | if 1==1 echo true                                | true,判断数字是否相等            |
| ■    | if not 1==2 echo true                            | true,判断数字是否不相等          |
| ■■   | if "a"=="a" echo true                            | true,判断字符串是否相等          |
| ■    | if /i "a"=="A" (echo true) else (echo false)     | true,字符串不区分大小写          |
|      | if a==a echo true                                | true,字符串可以不带引号          |
| ■■   | if "a"=="ab" (echo true) else (echo false)       | false,判断字符串是否相等         |
| ■■   | if exist test.bat echo true                      | true,判断文件是否存在            |
| ■■   | if defined a (echo true) else (echo false)       | false,判断变量是否定义           |
| ■■   | if 1 lss 2 (echo true) else (echo false)         | true,判断大小                    |
| ■■   | if "a" gtr "b" (echo true) else (echo false)     | false,判断大小                   |
| ■■   | if errorlevel 1 (echo true) else (echo false)    | false,错误代码是否大于等于1      |
