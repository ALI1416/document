...
3. 进入容器内部`docker exec -it mysql bash`查看要映射哪些内容
   1. 配置文件`/etc/my.cnf`映射到`/docker/mysql/conf/my.cnf`，创建文件夹`mkdir -p /docker/mysql/conf`，复制文件`docker cp nginx:/etc/my.cnf /docker/nginx/conf`
   2. 数据文件夹`/var/lib/mysql`映射到`/docker/mysql/data`，创建文件夹`mkdir -p /docker/mysql/data`
   3. 日志文件夹`/var/log/mysql`(不存在)映射到`/docker/mysql/log`，创建文件夹`mkdir -p /docker/mysql/log`
4. 退出容器`exit`，并执行命令

```sh
mkdir -p /docker/mysql/{conf,data,log}
docker cp mysql:/etc/my.cnf /docker/mysql/conf
```
