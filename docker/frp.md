...
3. 进入容器内部`docker exec -it frps sh`(使用了精简版linux，不能使用bash)查看要映射哪些内容
   1. 配置文件夹`/etc/frp`映射到`/docker/frps/conf`，复制文件夹`docker cp frps:/etc/frp /docker/frps/conf`
   2. 日志文件夹`/var/log/frp`(不存在)映射到`/docker/frps/log`，创建文件夹`mkdir -p /docker/frps/log`
4. 退出容器`exit`，并执行命令

```sh
mkdir -p /docker/frps/log
docker cp frps:/etc/frp /docker/frps/conf
```
...
8. 访问地址`http://127.0.0.1:7001/`有内容显示，即启动成功
